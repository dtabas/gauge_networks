...
def MPC(sys,x0):
    n,m,wb,x0b,x0_bar,Ad,Bd,r,u_bar,x_bar,Pt,bt,Pr,br,Pr_torch,br_torch = sys.sys_params
    Mu,Mw,M0,F,g,H0,Hw = sys.MPC_params
...
def generate_noise_train(batch,M,n,tau,T,wb,noise_type):
    W_traj = generate_noise_traj(batch,M,n,T,wb,noise_type)[1]
    j = range(batch*M)
    i = np.random.randint(0,(T-tau)*n,batch*M) + np.array(range(tau*n)).reshape((tau*n,1))
    return W_traj[i,j]
...
def train(sys,net,batch,M,lr=1e-3,max_epoch=200,noise_type='uniform'):
...
def test_rollout(sys,net,plot=False,noise_type='uniform'):
            
    n,m,wb,x0b,x0_bar,Ad,Bd,r,u_bar,x_bar,Pt,bt,Pr,br,Pr_torch,br_torch = sys.sys_params
    Q,R,QQ,RR = sys.cost_matrices
    ...
    for t in range(T):
        ...
        total_time_nn += t2-t1
        t3 = time.time()
        U_t_mpc,solve_time_mpc = MPC(sys,X_t_mpc)
        t4 = time.time()
        U_t_mpc = U_t_mpc[0:m]
...
def test_rollout_multiple(sys,net_list,noise_type='autoregressive',fig_num=100):
    ...
    plt.figure(fig_num,figsize=(8,4),dpi=1000)
    
    total_time_nn = 0
    total_time_mpc = 0
    
    if n == 2: 
        icv = torch.tensor([[1],[-1]])
    elif n == 3:
        icv = torch.tensor([[1],[-1],[1]])
    V0_torch = (torch.rand((n,1))*.25+.75)*icv #(torch.rand((n,1))-.5)*2
    X0_torch = gauge_map(torch.tensor(Pr).type(torch.FloatTensor),torch.tensor(br).type(torch.FloatTensor),V0_torch)
    X0_numpy = X0_torch.numpy() 
    ...
    for i,net in enumerate(net_list):
        ...
        plt.subplot(121)
        if n == 2:
            plt.plot(X_traj_nn[0,:],X_traj_nn[1,:],color[i])
        else:
            plt.step(range(T+1),X_traj_nn[1,:].T,color[i])
        plt.subplot(122)
        plt.step(range(T),U_traj_nn[0,:].T,color[i])
    ...
    plt.subplot(121)
    if n == 2:
        Polytope(Pr,br).plot(fill=False,edgecolor=(0,0,0),label='X')
        plt.plot(X_traj_mpc[0,:],X_traj_mpc[1,:],color[-1])
        plt.legend()
        plt.xlabel(r'$x^{(1)}$',font='Times',fontsize=15)
        plt.ylabel(r'$x^{(2)}$',font='Times',fontsize=15)
    else:
        plt.step(range(T+1),X_traj_mpc[1,:].T,color[-1])
        plt.xlabel('Time (s)',font='Times',fontsize=20)
        plt.ylabel('State',font='Times',fontsize=20)
    plt.xticks(font='Times',fontsize=15)
    plt.yticks(font='Times',fontsize=15)
    
    plt.subplot(122)
    plt.step(range(T),U_traj_mpc[0,:].T,color[-1])
    plt.legend(net_legend)
    plt.xlabel('Time (s)',font='Times',fontsize=20)
    plt.ylabel('Control',font='Times',fontsize=20)
    plt.xticks(font='Times',fontsize=15)
    plt.yticks(font='Times',fontsize=15)
    plt.tight_layout()
    plt.grid()
...
def test(sys,net,noise_type,N_test=N_test_standard):
    result_list = []
    #print('\n Testing:')
    for i in range(N_test):
        result = test_rollout(sys,net,plot=False,noise_type=noise_type)
...
@ray.remote
def test_once(sys,net,noise_type):
    result = test_rollout(sys,net,plot=False,noise_type=noise_type)
    return result

def test_parallel(sys,net,noise_type,N_test=N_test_standard):
    print('Testing')
    output_ids = []
    for i in range(N_test):
        output_ids.append(test_once.remote(sys,net,noise_type))
...
@ray.remote
def test_one_hyperparameter(sys,net_type,noise_type,i):
    ...
    n_epoch = 1
    if net_type == 'gauge':
        net = gauge_NN(sys,hidden_dim)
        max_epoch = n_epoch*N_epoch
    elif net_type == 'penalty':
        net = penalty_NN(sys,hidden_dim)
        max_epoch = n_epoch*N_epoch
    elif net_type == 'projection':
        net = projection_NN(sys,hidden_dim)
        max_epoch = n_epoch*N_epoch
    else:
        return
    train(sys,net,batch,M,lr=lr,max_epoch=max_epoch,noise_type=noise_type)
    
    result = test(sys,net,noise_type)
...
def tune_hyperparameters_parallel(sys,net_type = 'gauge'):
    
    aa = []
    net_list = []
    noise_type = 'autoregressive'
    if net_type == 'projection':
        N_search = 3###24 # Projection does well enough w/o hyperparameter tuning, and takes too long to train
    elif net_type == 'penalty':
        N_search = 3
    elif net_type == 'gauge':
        N_search = 3
    ...
    for i in range(N_search):
        output_ids.append(test_one_hyperparameter.remote(sys,net_type,noise_type,i))
    output_list = ray.get(output_ids)
...
@ray.remote
def test_one_hp_OL(sys,net_type,i):
    # For first iteration, select "good" hyperparameters
    if i == 0:
        hidden_dim = 2**8
        lr = 1e-4
    ...
    if net_type == 'gauge':
        ...
        return
    train(sys,net,batch,M,lr=lr,max_epoch=max_epoch,noise_type='autoregressive')
    
    performance = validate_OL(sys,net)
    a = [i,hidden_dim,lr,batch,M,performance]
...
def tune_hyperparameters_parallel_OL(sys,net_type):
    aa = []
    net_list = []
    if net_type == 'projection':
        N_search =3
    ...
    output_ids = []
    for i in range(N_search):
        output_ids.append(test_one_hp_OL.remote(sys,net_type,i))
    output_list = ray.get(output_ids)
    a3 = np.array([z[0] for z in output_list])
    net_list = [z[1] for z in output_list]
    ind = np.argmin(a3[:,-1])
...
class mpc_model(object):
    ...
    def __init__(self,tau):
        ...
        #n,m,wb,x0b,x0_bar,Ad,Bd,r,u_bar,x_bar,Pt,bt,Pr,br,Pr_torch,br_torch = sys.sys_params
        #Mu,Mw,M0,F,g,H0,Hw = sys.MPC_params
        #W,w,Fp2s,gp2s = sys.IP_params
        #K_weight,K_bias = sys.IP_matrices
        #Q,R,QQ,RR = sys.cost_matrices
...
if __name__ == '__main__':
    ...
    for net_type in nets:
        best_net,cost = tune_hyperparameters_parallel(sys,net_type=net_type)
        result = test_parallel(sys,best_net,noise_type='autoregressive',N_test=100)
    ...
    plt.figure(c+10,figsize=(8,4),dpi=1000)
    ax = plt.gca()
    prop_cycle = plt.rcParams['axes.prop_cycle']
    colors = prop_cycle.by_key()['color']
    color = colors[0:4]
    net_legend = ['Gauge NN','Penalty NN','Projection NN','Online MPC']
    #net_legend = ['Gauge NN','Penalty NN','Online MPC']
    ...
    plt.figure(c+2000,figsize=(8,4),dpi=1000)
    ax = plt.gca()
    prop_cycle = plt.rcParams['axes.prop_cycle']
    colors = prop_cycle.by_key()['color']
    color = colors[0:4]
    ...
    #nets = ['gauge','penalty']
    for net_type in nets:
        best_net,cost,hps = tune_hyperparameters_parallel_OL(sys,net_type=net_type)
        performance = validate_OL(sys,best_net)
